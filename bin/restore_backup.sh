...
BACKUP_FOLDER_PATH=`dirname $0`/../..
BACKUP_FILE_SQL_GZ=`ls -1 -t "${BACKUP_FOLDER_PATH}/*.bibliopuce.backup.sql.gz" | head -1`
...
BACKUP_FILE_SQL=`ls -1 -t "${BACKUP_FOLDER_PATH}/*.bibliopuce.backup.sql" | head -1`
