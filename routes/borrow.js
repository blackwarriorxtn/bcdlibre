...
'; \n' +
  (strSQLSearchValue == null
    ? /* No text - list all items */ 'INSERT IGNORE INTO tmp_item_search(id, relevance) \n' +
'SELECT id, 1 FROM item_detail_search \n' +
'; \n'

    : /* Search text - in three fields with 3 queries to optimize and ensure a proper key is used */ 'INSERT IGNORE INTO tmp_item_search(id, relevance) \n' +
...
router.get('/webservice/users', function (req, res, next) {
  ...
  if (req.query && req.query.action === 'borrow') {
    ...
    var arrSQL = [
      ...
      'FROM user_search ' + (strSQLWhere1 == null ? '' : '\nWHERE ' + strSQLWhere1) + '; \n'
    ]
...
'GROUP BY user.id \n' +
';')
    arrSQL.push('DROP TEMPORARY TABLE IF EXISTS tmp_search \n' + '; \n')
