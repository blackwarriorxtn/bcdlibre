...
function ModuleContext (req, res, next) {
  ...
  this.objFormParameters = {
    ...
    allowed_states: null,
    sql_counter: 'SELECT COUNT(item.id) AS counter, COUNT(borrow.id) AS borrowed\n' +
...
'GROUP BY item_detail_id'
  ...
  this.objSearchParameters = {
    ...
    fields: [
      {
        name: 'search',
        label: req.i18n.__('Titre, Auteur, Description'),
        type: 'String',
        required: true,
        validation: null,
        maximum_length: 255,
...
function awsPostProcessing (strISBN, objResultItem, objWebServiceResult) {
  debug('Amazon Web Service result: %j', objResultItem)
  var strNewValue = null
  if (objResultItem.Author) {
    strNewValue = objResultItem.Author
  ...
  if (objResultItem.Title) {
    ...
    strNewValue = objResultItem.Title
              ...
              db.handle_error(err, res, req, 'item/new',
                {
                  ...
                  menus: [objMyContext.objMainMenu].concat(objMyContext.objMenu),
                  form: objMyContext.objFormParameters,
                  message: {text: req.i18n.__("Ce livre est déjà dans l'inventaire (%s)", err), type: 'error'},
                  action: 'new'
                })
            } else {
                ...
                res.render('item/new',
                  {
                    ...
                    menus: [objMyContext.objMainMenu].concat(objMyContext.objMenu),
                    form: objMyContext.objFormParameters,
                    message: {text: req.i18n.__('Fiche ajoutée avec succès. Veuillez remplir la fiche suivante'), type: 'info'},
                    action: 'new'
                  })
...
router.post('/new_copy', function (req, res, next) {
  var objMyContext = new ModuleContext(req, res, next)
  var strItemDetailId = null
  var intItemDetailId = null
  if (req.body['_CANCEL'] != null) {
    // Cancel insert : Redirect to menu
    res.redirect('./')
  } else if (req.body['_OK'] != null) {
    // Always add at least ONE copy of the book (item => item_detail)
    strItemDetailId = req.body['id']
    intItemDetailId = parseInt(strItemDetailId, 10)
    db.runsql('INSERT INTO item(item_detail_id) VALUES(' + intItemDetailId.toString(10) + ');' /* strSQL */, function (err, arrRows, fields, objSQLConnection) {
      ...
      res.render('item/new', {
        ...
        menus: [objMyContext.objMainMenu].concat(objMyContext.objMenu),
        form: objMyContext.objFormParameters,
        message: {text: req.i18n.__('Exemplaire ajouté avec succès. Veuillez remplir la fiche suivante'), type: 'info'},
        action: 'new'})
...
router.get('/view', function (req, res, next) {
  ...
  db.view_record(req, res, next, objMyContext.objFormParameters, function (err, arrRows, fields) {
    ...
    // TODO DELETE var strURLCopies = 'new_copy?item_detail_id=' + encodeURIComponent(result.id)
    // TODO DELETE var strURLCopiesAdd = strURLCopies + '&action=' + encodeURIComponent('1')
    // TODO DELETE var strURLCopiesRemove = strURLCopies + '&action=' + encodeURIComponent('-1')
      ...
      for (var intResult in res) {
        ...
        if (res[intResult].stat) {
          ...
          if (res[intResult].stat === 'ok' && res[intResult].list && res[intResult].list.length > 0) {
            ...
            if (objResultItem.author) {
              strNewValue = objResultItem.author // TODO Handle multiple authors?
              // WARNING: don't overwrite result from another Web Service, unless this result is longuer (ok, it's a silly way to finding the most appropriate answer)
              if (objWebServiceResult.authors == null || objWebServiceResult.authors.length < strNewValue.length) {
                objWebServiceResult.authors = strNewValue
              }
          ...
          if (res[intResult].totalItems > 0 && res[intResult].items.length && res[intResult].items[0].volumeInfo) {
            objResultItem = res[intResult].items[0].volumeInfo // TODO Handle multiple results? (let the user pick one?)
            if (objResultItem.authors) {
              strNewValue = objResultItem.authors.join(', ')
              // WARNING: don't overwrite result from another Web Service, unless this result is longuer (ok, it's a silly way to finding the most appropriate answer)
              if (objWebServiceResult.authors == null || objWebServiceResult.authors.length < strNewValue.length) {
                objWebServiceResult.authors = strNewValue
              }
          ...
          if (res[intResult].ItemLookupResponse && res[intResult].ItemLookupResponse.Items && res[intResult].ItemLookupResponse.Items.Item) {
            ...
            if (objResults.length == null || objResults.length === 0) {
              ...
              awsPostProcessing(strISBN, objResults.ItemAttributes, objWebServiceResult)
            } else {
              ...
              for (var intItem = 0; intItem < objResults.length; intItem++) {
                objResultItem = objResults[intItem].ItemAttributes
                awsPostProcessing(strISBN, objResultItem, objWebServiceResult)
              } // for (var intItem = 0; intItem < objResults.length; intItem++)
            } // if (objResults.length == null || objResults.length == 0)
          } else {
          ...
          if (res[intResult].GoodreadsResponse.search &&
            ...
            objResultItem = res[intResult].GoodreadsResponse.search[0].results[0].work[0].best_book[0] // TODO Handle multiple results? (let the user pick one?)
            if (objResultItem && objResultItem.author && objResultItem.author.length > 0 && objResultItem.author[0].name) {
              strNewValue = objResultItem.author[0].name[0] // TODO Handle multiple authors?
              // WARNING: don't overwrite result from another Web Service, unless this result is longuer (ok, it's a silly way to finding the most appropriate answer)
              if (objWebServiceResult.authors == null || objWebServiceResult.authors.length < strNewValue.length) {
                objWebServiceResult.authors = strNewValue
              }
...
router.get('/webservice/items', function (req, res, next) {
  ...
  if (req.query && req.query.action === 'classification') {
    ...
    var arrSQL = [
      ...
      '  FROM item_classification' + (strSQLWhere1 == null ? '' : '\nWHERE ' + strSQLWhere1) + '; \n'
    ]
...
router.get('/webservice/img', function (req, res, next) {
  ...
  if (req.query.url) {
    ...
    if (fs.existsSync(strImageFilePath)) {
      ...
      itemImageSaveToLocalCache(req.query.url, strImageFilePath, intItemDetailId)
    }
  } else {
...
function itemImageSaveToLocalCache (strImageURL, strImageFilePath, intItemDetailId) {
  ...
  request.head(strImageURL,
    function (err, res, body) {
      if (err) {
        ...
        // TODO DELETE var strDate = res.headers['date']
        // TODO DELETE var strLastModifided = res.headers['last-modified']
        ...
        request(
          {
            url: strImageURL,
            encoding: null /* We expect binary data */
          }).pipe(fs.createWriteStream(strImageFilePath))
      }
    }
  )
}
