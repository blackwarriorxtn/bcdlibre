...
CREATE
  ...
  VIEW borrow_list
AS
  SELECT borrow.*, item.item_detail_id, item_detail.title, item_detail.author, user.first_name, user.last_name, user.category, DATEDIFF(NOW(),borrow.begin_date) AS duration_in_days
  FROM borrow
